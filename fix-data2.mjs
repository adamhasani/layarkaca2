...
for (let i = 0; i < blocks.length; i++) {
  if (blocks[i].includes('upload.wikimedia.org')) {
    ...
    if (titleMatch) {
      ...
      if (data.results && data.results.length > 0) {
        ...
        if (p) {
          blocks[i] = blocks[i].replace(/posterUrl:\s*'[^']+'/, `posterUrl: 'https://image.tmdb.org/t/p/w500${p}'`);
          const banner = b ? `https://image.tmdb.org/t/p/original${b}` : `https://image.tmdb.org/t/p/w500${p}`;
          blocks[i] = blocks[i].replace(/bannerUrl:\s*'[^']+'/, `bannerUrl: '${banner}'`);
...
fs.writeFileSync('src/data.ts', blocks.join('{'), 'utf8');
